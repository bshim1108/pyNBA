...
class TPSModel(object):
    def __init__(self, train_data, test_data):
        ...
        self.model = CatBoostRegressionModel(TPS_MODEL_PARAMS)
    ...
    def create_features(self, odds_data, sp_threshold=1200):
        ...
        data[self.regressand] = data['TOV']/data['SECONDSPLAYED']
        ...
        data = self.feature_creation.rolling_weighted_mean(
            df=data, group_col_names=['SEASON', 'TEAM', 'PLAYERID'], col_name=self.regressand, new_col_name='MA26_Y',
            weight_col_name='SECONDSPLAYED', n_rolling=26, min_periods=13
        )
        self.regressors.append('MA26_Y')
        ...
        data = self.feature_creation.expanding_mean(
            df=data, group_col_names=['SEASON', 'TEAM', 'PLAYERID'], col_name='TOV', new_col_name='AVG_TOV'
        )
        data = self.feature_creation.expanding_mean(
            df=data, group_col_names=['SEASON', 'TEAM', 'PLAYERID'], col_name='SECONDSPLAYED', new_col_name='AVG_SP'
        )

        temp = data.dropna(subset=['TOV', 'SECONDSPLAYED', 'AVG_TOV', 'AVG_SP'])
    ...
    def preprocess(self, data):
        ...
        data['MA26_Y'] = data['MA26_Y'].fillna(data['MA6_Y'])

...
from pyNBA.Models.constants import SPS_MODEL_PARAMS


class SPSModel(object):
    ...
    def create_features(self, odds_data, sp_threshold=60):
        ...
        data = self.feature_creation.rolling_weighted_mean(
            df=data, group_col_names=['SEASON', 'TEAM', 'PLAYERID'], col_name=self.regressand, new_col_name='MA4_Y',
            weight_col_name='SECONDSPLAYED', n_rolling=4, min_periods=2
        )
        data = self.feature_creation.rolling_weighted_mean(
            df=data, group_col_names=['SEASON', 'TEAM', 'PLAYERID'], col_name=self.regressand, new_col_name='MA10_Y',
            weight_col_name='SECONDSPLAYED', n_rolling=10, min_periods=5
        )
        self.regressors.append('MA2_Y')
        self.regressors.append('MA4_Y')
        self.regressors.append('MA10_Y')
        ...
        data = self.feature_creation.expanding_mean(
            df=data, group_col_names=['SEASON', 'TEAM', 'PLAYERID'], col_name='STL', new_col_name='AVG_STL'
        )
        data = self.feature_creation.expanding_mean(
            df=data, group_col_names=['SEASON', 'TEAM', 'PLAYERID'], col_name='SECONDSPLAYED', new_col_name='AVG_SP'
        )

        temp = data.dropna(subset=['STL', 'SECONDSPLAYED', 'AVG_STL', 'AVG_SP'])
    ...
    def preprocess(self, data):
        ...
        data['MA4_Y'] = data['MA4_Y'].fillna(data['MA2_Y'])
        data['MA10_Y'] = data['MA10_Y'].fillna(data['MA4_Y'])

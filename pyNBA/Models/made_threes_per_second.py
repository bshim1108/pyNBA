...
class MTPSModel(object):
    def __init__(self, train_data, test_data):
        ...
        self.model = CatBoostRegressionModel(MTPS_MODEL_PARAMS)

        self.regressors = []
        self.regressand = 'MTPS'
    ...
    def create_features(self, odds_data, sp_threshold=1200):
        ...
        data[self.regressand] = data['FG3M']/data['SECONDSPLAYED']
        ...
        data['ATPS'] = data['FG3A']/data['SECONDSPLAYED']
        data = self.feature_creation.expanding_weighted_mean(
            df=data, group_col_names=['SEASON', 'TEAM', 'PLAYERID'], col_name='ATPS', new_col_name='AVG_ATPS', weight_col_name='SECONDSPLAYED'
        )
        data = self.feature_creation.lag(
            df=data, group_col_names=['SEASON', 'TEAM', 'PLAYERID'], col_name='ATPS', new_col_name='L1_ATPS', n_shift=1
        )
        data = self.feature_creation.rolling_weighted_mean(
            df=data, group_col_names=['SEASON', 'TEAM', 'PLAYERID'], col_name='ATPS', new_col_name='MA3_ATPS',
            weight_col_name='SECONDSPLAYED', n_rolling=3, min_periods=2
        )
        self.regressors.append('AVG_ATPS')
        self.regressors.append('L1_ATPS')
        self.regressors.append('MA3_ATPS')

        data['FG3%'] = data['FG3M']/data['FG3A']
        data = self.feature_creation.expanding_weighted_mean(
            df=data, group_col_names=['SEASON', 'TEAM', 'PLAYERID'], col_name='FG3%', new_col_name='AVG_FG3%', weight_col_name='FG3A'
        )
        data = self.feature_creation.rolling_weighted_mean(
            df=data, group_col_names=['SEASON', 'TEAM', 'PLAYERID'], col_name='FG3%', new_col_name='MA3_FG3%',
            weight_col_name='FG3A', n_rolling=3, min_periods=2
        )
        self.regressors.append('AVG_FG3%')
        self.regressors.append('MA3_FG3%')

        # defense
        data = self.feature_creation.expanding_mean(
            df=data, group_col_names=['SEASON', 'TEAM', 'PLAYERID'], col_name='FG3M', new_col_name='AVG_FG3M'
        )
        data = self.feature_creation.expanding_mean(
            df=data, group_col_names=['SEASON', 'TEAM', 'PLAYERID'], col_name='SECONDSPLAYED', new_col_name='AVG_SP'
        )

        temp = data.dropna(subset=['FG3M', 'SECONDSPLAYED', 'AVG_FG3M', 'AVG_SP'])
        ...
        # injuries
        data = self.feature_creation.expanding_mean(
            df=data, group_col_names=['SEASON', 'TEAM', 'PLAYERID'], col_name='FG3A', new_col_name='AVG_FG3A'
        )
        ...
        data = self.feature_creation.expanding_weighted_mean(
            df=data, group_col_names=['SEASON', 'START_LINEUP', 'PLAYERID'], col_name='ATPS', weight_col_name='SECONDSPLAYED',
            new_col_name='AVG_ATPS_STARTERS'
        )
        data = self.feature_creation.expanding_weighted_mean(
            df=data, group_col_names=['SEASON', 'STARS', 'PLAYERID'], col_name='ATPS', weight_col_name='SECONDSPLAYED',
            new_col_name='AVG_ATPS_STARS'
        )
        self.regressors.append('AVG_Y_STARTERS')
        self.regressors.append('AVG_Y_STARS')
        self.regressors.append('AVG_ATPS_STARTERS')
        self.regressors.append('AVG_ATPS_STARS')
    ...
    def preprocess(self, data):
        ...
        data['L1_ATPS'] = data['L1_ATPS'].fillna(data['AVG_ATPS'])
        data['MA3_ATPS'] = data['MA3_ATPS'].fillna(data['AVG_ATPS'])

        data['AVG_FG3%'] = data['AVG_FG3%'].fillna(data['AVG_FG3%'].mean())
        data['MA3_FG3%'] = data['MA3_FG3%'].fillna(data['AVG_FG3%'])
        ...
        data['TEAM_ACTIVE_AVG_ATPS_DIFF'] = data['TEAM_ACTIVE_AVG_ATPS_DIFF'].fillna(0)

        data['AVG_Y_STARS'] = data['AVG_Y_STARS'].fillna(data['AVG_Y'])
        data['AVG_Y_STARTERS'] = data['AVG_Y_STARTERS'].fillna(data['AVG_Y_STARS'])
        data['AVG_ATPS_STARS'] = data['AVG_ATPS_STARS'].fillna(data['AVG_ATPS'])
        data['AVG_ATPS_STARTERS'] = data['AVG_ATPS_STARTERS'].fillna(data['AVG_ATPS_STARS'])

...
class FeatureCreation(object):
    ...
    def expanding_mean(self, df, group_col_names, col_name, new_col_name, min_periods=1, n_shift=1, order_idx_name='GAMEID'):
        ...
        df = self.merge_stat_to_df(stat_df=temp, df=df, group_col_names=group_col_names, col_name=col_name, new_col_name=new_col_name, n_shift=n_shift)
        
        if original_idx_name is None:
            return df
        return df.set_index(original_idx_name)

    def expanding_weighted_mean(self, df, group_col_names, col_name, weight_col_name, new_col_name, min_periods=1, n_shift=1, order_idx_name='GAMEID'):
        df, original_idx_name = self.prepare_df(df, new_col_name=new_col_name, order_idx_name=order_idx_name)

        df['weighted_col'] = df[col_name] * df[weight_col_name]
        temp = df.groupby(group_col_names)
        temp = temp['weighted_col'].expanding(min_periods=min_periods).sum() / temp[weight_col_name].expanding(min_periods=min_periods).sum()
    ...
    def expanding_ewm(self, df, group_col_names, col_name, new_col_name, alpha, min_periods=1, n_shift=1, order_idx_name='GAMEID'):
        df, original_idx_name = self.prepare_df(df, new_col_name=new_col_name, order_idx_name=order_idx_name)

        temp = df.set_index(group_col_names, append=True)
        temp = temp.groupby(group_col_names)[col_name].transform(lambda x: x.ewm(alpha=alpha, min_periods=min_periods).mean())
    ...
    def expanding_sum(self, df, group_col_names, col_name, new_col_name, min_periods=1, n_shift=1, order_idx_name='GAMEID'):
        df, original_idx_name = self.prepare_df(df, new_col_name=new_col_name, order_idx_name=order_idx_name)
    ...
    def rolling_mean(self, df, group_col_names, col_name, new_col_name, n_rolling, min_periods=None, n_shift=1, order_idx_name='GAMEID'):
    ...
    def rolling_weighted_mean(self, df, group_col_names, col_name, weight_col_name, new_col_name, n_rolling, min_periods=None, n_shift=1, order_idx_name='GAMEID'):
        min_periods = min_periods or n_rolling
        df, original_idx_name = self.prepare_df(df, new_col_name=new_col_name, order_idx_name=order_idx_name)
        ...
        df['weighted_col'] = df[col_name] * df[weight_col_name]
        temp = df.groupby(group_col_names)
        temp = temp['weighted_col'].rolling(window=n_rolling, min_periods=min_periods).sum() / temp[weight_col_name].rolling(n_rolling, min_periods=min_periods).sum()
        temp = temp.rename(col_name)
        df = df.drop(columns='weighted_col')
    ...
    def standard_deviation(self, df, group_col_names, col_name, new_col_name, min_periods=1, n_shift=1, order_idx_name='GAMEID'):
        df, original_idx_name = self.prepare_df(df, new_col_name=new_col_name, order_idx_name=order_idx_name)

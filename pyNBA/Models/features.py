...
class FeatureCreation(object):
    ...
    def merge_stat_to_df(self, stat_df, df, group_col_names, col_name, new_col_name, n_shift):
    ...
    def prepare_temp(self, df, group_col_names, order_idx_name):
        temp = df.reset_index()
        temp = temp.drop_duplicates(subset=group_col_names + [order_idx_name])
        temp = temp.set_index(order_idx_name)
        return temp
    ...
    def expanding_mean(self, df, group_col_names, col_name, new_col_name, min_periods=1, n_shift=1,
        ...
        temp = temp.groupby(group_col_names)[col_name].expanding(min_periods=min_periods).mean()
    ...
    def expanding_weighted_mean(self, df, group_col_names, col_name, weight_col_name, new_col_name, min_periods=1,
        ...
        temp = self.prepare_temp(df, group_col_names, order_idx_name)
        temp['weighted_col'] = temp[col_name] * temp[weight_col_name]
        temp.loc[
    ...
    def expanding_ewm(self, df, group_col_names, col_name, new_col_name, alpha, min_periods=1, n_shift=1,
        ...
        temp = self.prepare_temp(df, group_col_names, order_idx_name)
        temp = temp.set_index(group_col_names, append=True)
    ...
    def expanding_sum(self, df, group_col_names, col_name, new_col_name, min_periods=1, n_shift=1,
        ...
        temp = self.prepare_temp(df, group_col_names, order_idx_name)
        temp = temp.groupby(group_col_names)[col_name].expanding(min_periods=min_periods).sum()
    ...
    def rolling_mean(self, df, group_col_names, col_name, new_col_name, n_rolling, min_periods=None, n_shift=1,
        ...
        temp = temp.groupby(group_col_names)[col_name].rolling(window=n_rolling, min_periods=min_periods).mean()
    ...
    def lag(self, df, group_col_names, col_name, new_col_name, n_shift=1, order_idx_name='DATE'):
        ...
        temp = self.prepare_temp(df, group_col_names, order_idx_name)
        temp = temp.set_index(group_col_names, append=True)[col_name]
    ...
    def expanding_standard_deviation(self, df, group_col_names, col_name, new_col_name, min_periods=1,
        ...
        temp = self.prepare_temp(df, group_col_names, order_idx_name)
        temp = temp.groupby(group_col_names)[col_name].expanding(min_periods=min_periods).std()

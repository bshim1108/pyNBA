...
class MinutesPlayed(object):
    ...
    def generate_regressors(self, boxscores, start_date, end_date):
        ...
        player_to_mintes = Helpers().get_player_minutes()

        boxscores['MINUTES_PROJECTION'] = boxscores['NAME'].apply(lambda x: player_to_mintes[x] if x in player_to_mintes else -0.01)
    ...
    def predict(self, boxscores, predict_start_date, predict_end_date, update=True):
        ...
        predicted_data['AVG_MP(REG)_R'] = 0

        cols = [
            'SEASON', 'DATE', 'TEAM', 'OPP_TEAM', 'NAME', 'POSITION', 'START', 'PLAYERCHANCE',
            'MINUTES_PROJECTION', 'AVG_MP(REG)_R'
            ]

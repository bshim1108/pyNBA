...
class RandomObjective(OptimizerRule):
    def apply_for_iteration(self, solver, players_dict, result):
        variables = []
        coefficients = []
        optimizer_standard_deviation = self.optimizer.get_standard_deviation()
        optimizer_max_deviation = self.optimizer.get_max_deviation()
        for player, variable in players_dict.items():
            variables.append(variable)
            multiplier = normal(
                0,
                player.standard_deviation if player.standard_deviation is not None else optimizer_standard_deviation
            )
            if player.max_deviation is not None:
                multiplier = sign(multiplier) * min(abs(multiplier), player.max_deviation)
            else:
                multiplier = sign(multiplier) * min(abs(multiplier), optimizer_max_deviation)
            coefficients.append(player.fppg * (1 + multiplier))
        solver.set_objective(variables, coefficients)
